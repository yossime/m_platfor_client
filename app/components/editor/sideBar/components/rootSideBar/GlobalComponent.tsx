...
// import {  buttonStyleOptions, imageStyleOptions, textSizeOptions } from "../../types";
...
//   const { currentEnvironment,setCurrentEnvironment ,textures ,filePaths} = useEnvironmentContext();
...
export const GlobalComponent: React.FC = () => {
  const { currentEnvironment, setCurrentEnvironment, textures, filePaths, loadTexturesByIndices } = useEnvironmentContext();
    ...
    <Container>
      <ContainerStyle>
        ...
        {filePaths.map((path, index) => (
          <ItemRow key={index} onClick={() => handlePathClick(index)}>
            <Text>{path}</Text>
      ...
      <ContainerStyle>
        ...
        {textures.map((texture, index) => (
          <ItemRow key={index} onClick={() => handleTextureClick(index)}>
            <ItemImage src={texture.image.src} alt={`Texture ${index}`} />
            <Text>Texture {index + 1}</Text>
